...
def parse_html(html_fragment: str, ws: Worksheet) -> None:
    ...
    global FOLDER_NAME
    FOLDER_NAME = soup.find('h1', id='folderName').text
...
def main(folder_name: str = None) -> None:
    ...
    for html_fragment in tqdm(get_html_fragment(folder_name), desc="Processing HTML fragments"):
        parse_html(html_fragment, ws)

    # Save the workbook with the specified folder name and current date
    wb.save(f'{folder_name}-{today}.xlsx')
