...
def main(user_id=None, folder_id=None, folder_name='Watched'):
    """
    ...
    # Define the base URL and retrieve necessary information
    ...
    elif user_id is not None and folder_id is None:
    ...
    cookies: dict = get_cookies_from_file()  # TODO try to get cookies from site
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        ...
        for future in tqdm(as_completed(futures), total=pages_count, desc="Downloading pages"):
            try:
                response, page = future.result()  # Check if the future completed successfully
                save_html(f'{path_folder}/{page}.html', response)  # TODO folder name
